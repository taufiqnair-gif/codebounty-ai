...
import { getBounties, submitFix } from "@/lib/bounty-service"
import type { Bounty } from "@/types/bounty"
import { useWallet } from "@/hooks/use-wallet"
import { Textarea } from "@/components/ui/textarea"
import CodeEditor from "@/components/code-editor"
import { Loader2 } from "lucide-react"
import { BountyStatus } from "@/types/bounty" // Declare the BountyStatus variable
...
export default function BountiesPage() {
  ...
  const handleSubmitFix = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedBounty || !fixCode || !fixDescription || !isConnected) return

    setIsSubmitting(true)
    try {
      await submitFix(selectedBounty.id, fixCode, fixDescription)
      toast({
        title: "Fix Submitted",
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "Completed":
        return <Badge className="bg-purple-500/20 text-purple-500 border-purple-500">Completed</Badge>
      default:
      ...
      <div className="container py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card className="border-gray-800/50 bg-gray-900/20">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white">
                {bounties.reduce((sum, b) => sum + b.reward, 0).toFixed(2)} ETH
              </div>
          ...
          <Card className="border-gray-800/50 bg-gray-900/20">
            <CardContent className="p-6 text-center">
              ...
              <div className="text-2xl font-bold text-white">{bounties.filter((b) => b.status === "Open").length}</div>
        ...
        <Tabs defaultValue="open" className="mb-8">
          ...
          <TabsContent value="my" className="mt-6">
            {!isConnected ? (
            ...
            ) : bounties.filter((b) => b.hunter === address).length === 0 ? (
              <div className="text-center py-16">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {bounties
                  .filter((b) => b.hunter === address)
                  .map((bounty) => (
                    <Card key={bounty.id} className="border-gray-800/50 bg-gray-900/20">
                      <CardHeader>
                        <div className="flex justify-between items-start">
                          <CardTitle className="text-lg font-medium text-white">{bounty.title}</CardTitle>
                          {getStatusBadge(bounty.status)}
                        </div>
                        <CardDescription className="text-gray-400">{bounty.description}</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-400">Reward</span>
                            <div className="font-bold text-green-400">{bounty.reward} ETH</div>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-gray-400">Submission Date</span>
                            <div className="text-sm text-gray-300">
                              {new Date(bounty.submissionDate || Date.now()).toLocaleDateString()}
                            </div>
                          </div>
                        </div>
                      </CardContent>
            ...
            <CardContent>
              <form onSubmit={handleSubmitFix} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="p-4 rounded-lg bg-red-500/10 border border-red-500/20">
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <h4 className="font-medium text-red-500">High Severity Issues</h4>
                        <ul className="mt-2 space-y-1 text-sm text-gray-300">
                          {selectedBounty.vulnerabilities
                            ?.filter((v) => v.severity === "High")
                            .map((vuln, i) => (
                              <li key={i}>
                                <span className="font-medium">{vuln.type}</span> - Line {vuln.line}
                              </li>
                            ))}
                        </ul>
                      </div>
                ...
                <div className="flex justify-end space-x-3">
                  <Button type="button" variant="outline" onClick={() => setSelectedBounty(null)}>
                    Cancel
                  </Button>
                  <Button
                    type="submit"
